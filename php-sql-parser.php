...
EOREGEX
		...
		private function process_select_expr($expression) {
			...
			preprint("base_expr:\n".preprint($base_expr, true));
			preprint("stripped before:\n".preprint($stripped, true));

			$stripped = $this->process_expr_list($stripped);

			preprint("stripped after:\n".preprint($stripped, true));
			...
			/* FIXME: we have removed it, because the expression list can parse subqueries too
			 # we have an expression, which is a select statement within parenthesis
			 # like: select (select colA from test) as b from example
			 if(substr(trim($base_expr),0,1) == '(') {
				# the "expression" contains the possible alias
				# we should not use it, the base_expr seems to be enough
				# but it fails on select (select colA from tableA) + (select colB from tableB) from tableC
				# TODO: fix that, maybe we can use "stripped"
				$base_expr = substr(trim($base_expr),1,-1);
				if(preg_match('/^select\\s/i', $base_expr)) {
				$type='subquery';
			...
			private function process_from(&$tokens) {
				...
				foreach($tokens as $token) {
					...
					switch($upper) {
						...
						case 'JOIN':
							...
							# removed AR, but it goes wrong if we have 
							# select * from e where e.id=0 -> this uses no alias on table, but a full qualified tablename on column!
				...
				if(substr(trim($table),0,1) == '(') {
					...
					$alias = "";
					#			} else {
					# this defines the tablename as alias
					#				if(!trim($alias)) $alias = $table;
				}
